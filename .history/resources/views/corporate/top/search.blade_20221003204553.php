<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div>
                    <form action="{{route('corporate.search')}}" method="GET">
                        希望職種：<input type="checkbox" name="keyword1" value="フロントエンドエンジニア"> フロントエンドエンジニア
                        ...
                        <input type="checkbox" name="keyword1" valueH="ITアーキテクト"> ITアーキテクト<br><br>
                        ...
                        <input type="submit" value="検索" style="background-color:#e5e7eb">
                    </form>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <table>
                        ...
                        <tr>
                            <td>{{$engineer_info->user->engineer_name}}<br>
                                @if ($engineer_info->user->nice)
                                受け取ったいいね数:{{$engineer_info->user->nice->count()}}&emsp;&emsp;</td>
                                @else
                                受け取ったいいね数:0&emsp;&emsp;</td>
                                @endif
                            ...
                            @if($engineer_info->user->nice::where('engineer_name',$engineer_info->user->engineer_name) && $engineer_info->user->nice::where('name',$user_name) && $engineer_info->user->engineer_name==="takaki")
                            <td><a href="{{route('corporate.unnice',['id'=>$engineer_info->user_id])}}">いいね済&emsp;&emsp;</a></td>
                            @else
                            <td><a href="{{route('corporate.nice',['id'=>$engineer_info->user_id])}}">いいね&emsp;&emsp;</a></td>
                            @endif
                            <td>詳細</td>
                        </tr>
                        @endforeach
    ...
    <script>
        function selectionResult() {
            let em = document.getElementById("age");
            const age=em.value;
            console.log('xxx',age);
            ...
            .then(function(result) {
                ...
                ageResult.textContent=result.count;
            });
        }

        let em = document.getElementById("age");

        em.addEventListener('change', selectionResult);
    </script>
</x-app-layout>
