<x-app-layout>
    ...
    <script>
        function selectionResult() {
            var em = document.getElementById("age");
            const age=em.value;
            console.log('xxx',age);

            const url='http://127.0.0.1:8000/api/ageResult/'+age;
            let response = await (url);
            let result = await response.json(); // レスポンスの本文を読み JSON としてパースする
            result.count;

            fetch(url);
            .then(funcction(response){response.json()});
