...
{
    public function corporateSearch(Request $request,$id,$sort){
        \Log::debug('[SearchController,corporateSearch],id=>',[$id]);
        \Log::debug('[SearchController,corporateSearch],id=>',[$sort]);
        ...
        $recordCount=2;
        $offsetCount=0;
        $offsetCount=($id-1)*$recordCount;

        $engineer_infos=$query->get();
        //pageCount用の配列。->limit前に記載することで$pageCountListを担保
        $count=0;
        $pageCount=1;
        $pageCountList=[1];
        ...
        $engineer_infos=$query->limit($recordCount)->offset($offsetCount)->get();
