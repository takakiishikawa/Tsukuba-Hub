...
class SearchController extends Controller
{
    ...
    public function result(){
        ...
        if ($data['keyword']==='keyword1'){
            $query->where('objective',$data['value'])->get();
        }else ($data['keyword']==='keyword2'){
            $query->where('work_experience',$data['value'])->get();
        }else ($data['keyword']==='keyword4'){
            $query->where('development_language',$data['value'])->get();
        }else ($data['keyword']==='keyword5'){
            $query->where('development_framework',$data['value'])->get();
        }else ($data['keyword']==='keyword6'){
            $query->where('development_database',$data['value'])->get();
        }else ($data['keyword']==='keyword7'){
            $query->where('development_infra',$data['value'])->get();
        }else ($data['keyword']==='keyword8'){
            $query->where('development_os',$data['value'])->get();
        }else ($data['keyword']==='keyword9'){
            $query->where('expectation',$data['value'])->get();
        }else ($data['keyword']==='keyword10'){
            $query->where('strength',$data['value'])->get();
        }else ($data['keyword']==='keyword11'){
            $query->where('strength',$data['value'])->get();
        }
