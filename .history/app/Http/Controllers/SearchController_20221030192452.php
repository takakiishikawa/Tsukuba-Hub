...
class SearchController extends Controller
{
    ...
    public function result(){
        ...
        if ($item['keyword']==='keyword1'){
            $query->where('objective',$item['value'])->get();
        }
        if ($item['keyword']==='keyword2'){
            $query->where('work_experience',$item['value'])->get();
        }
        if ($item['keyword']==='keyword4'){
            $query->where('development_language',$item['value'])->get();
        }
        if ($item['keyword']==='keyword5'){
            $query->where('development_framework',$item['value'])->get();
        }
        if ($item['keyword']==='keyword6'){
            $query->where('development_database',$item['value'])->get();
        }
        if ($item['keyword']==='keyword7'){
            $query->where('development_infra',$item['value'])->get();
        }
        if ($item['keyword']==='keyword8'){
            $query->where('development_os',$item['value'])->get();
        }
        if ($item['keyword']==='keyword9'){
            $query->where('expectation',$item['value'])->get();
        }
        if ($item['keyword']==='keyword10'){
            $query->where('strength',$item['value'])->get();
        }
