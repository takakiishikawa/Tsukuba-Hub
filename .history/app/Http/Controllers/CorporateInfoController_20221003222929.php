...
class CorporateInfoController extends Controller
{
    ...
    public function create(Request $request){
        ...
        public function create(Request $request){
            \Log::debug('[FilemanageController][create]');
            $title = $request->input('title');      //タイトル
            $memo = $request->input('memo');        //メモ
            $uploadedfiles = $request->file('file'); //ファイル本体

            $filename = $uploadedfile->getClientOriginalName(); //ブラウザ側で知りえたオリジナルのファイル名
            \Log::debug('[FilemanageController][create]',[$title,$memo,$filename]);
            ...
            //ストレージにファイル保存
            $uploadedfile->storeAs('',$fileitem->id);       //認証ありなどカスタマイズ可能にする。''は保存フォルダ名
            // $uploadedfile->storeAs('public',$fileitem->id);  //こちらは認証無しで見えてしまう保存方法
