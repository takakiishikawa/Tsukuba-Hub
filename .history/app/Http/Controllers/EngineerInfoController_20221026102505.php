...
class EngineerInfoController extends Controller
{
    public function index(){
        ...
        public function getfile(Request $request , $id ){
            $fileitem = FileItem::find($id);    // ファイルアイテムをDBから探す
            $storedfilename = $fileitem->id;    // 保存したファイル名を id から類推
            $filename = $fileitem->filename;    // オリジナルのファイル名を取得
            $mimeType = Storage::mimeType($storedfilename); //ファイルのMIMEタイプを取得
            $headers = [['Content-Type' => $mimeType]];     //ヘッダに付け加える
            return Storage::response($storedfilename,$filename,$headers); // ファイル本体を返却する
        }
