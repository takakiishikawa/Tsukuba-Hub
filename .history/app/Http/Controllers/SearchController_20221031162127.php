...
class SearchController extends Controller
{
    public function corporateSearch(Request $request){
        ...
        foreach($myNiceRecords as $myNiceRecord){
            $niceList[$myNiceRecord->user_id]=$myNiceRecord;
        }

        $myNicedRecords=Nice::where('name',$user_name)->where('type',"エンジニア")->get();
        $nicedList=[];
        foreach($myNicedRecords as $myNicedRecord){
            $tmp=$myNicedRecord->engineer_name;
            $xxx=User::where('engineer_name',$tmp)->first();

            $nicedList[$myNicedRecord->user_id]=$myNicedRecord->engineer_name;
        }
        \Log::debug($nicedList);
