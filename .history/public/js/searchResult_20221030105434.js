        ...
        */
        ...
        const searchChangedElms2=document.querySelectorAll('[name="keyword2"]');
        searchResult2();
        ...
        async function searchResult2(){
            searchChangedElms2.forEach(function(searchChangedElm2){
                searchChangedElm2.addEventListener('change',()=>{
                    const data=document.querySelector('input:checked').value;
                    const url='http://127.0.0.1:8000/api/Result/'+[checkedValue=>data];
                    console.log(url);
